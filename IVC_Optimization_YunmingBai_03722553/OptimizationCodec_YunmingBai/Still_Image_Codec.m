% StillImageCodec
% output: distortion_stillImage
%         Rate_stillImage
tic;

scales =[0.1,0.2, 0.4, 0.8, 1.0, 1.5, 2, 3, 4, 4.5,5,5.5,6 , 7, 8]; 
for scaleIdx = 1 : numel(scales)
    ...
    bitPerPixel(i) = (numel(bytestream)*8) / (numel(current_frame)/3);
    %% image reconstruction
    
    k_rec = double(dec_huffman_new ( bytestream, BinaryTree,  size( current_frame_introencode(:),1 ) ))+a-1;
    I_rec = IntraDecode(k_rec, size(current_frame),qScale);
...
distortion_stillImage(scaleIdx) = mean_PSNR;
Rate_stillImage(scaleIdx) = means_rate;
end

save('distortion_stillImage.mat','distortion_stillImage');
save('rate_stillImageCodec.mat','Rate_stillImage');
